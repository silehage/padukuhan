...
import {  guard, moneyFormat } from '@/lib/utils';
                    ...
                    <table class="table bordered">
                        <thead>
                            <tr>
                                ...
                                <th class="text-right uppercase">Penerimaan</th>
                                <th class="text-right uppercase">Pengeluaran</th>
                        ...
                        <tbody>
                            <tr v-for="(item, idx) in data.items" :key="idx">
                                ...
                                <td class="text-right">
                                    {{ item.in_out == 'IN' ? moneyFormat(item.jumlah) : '' }}
                                </td>
                                <td class="text-right">{{ item.in_out == 'OUT' ? moneyFormat(item.jumlah) : '' }}</td>
                                <!-- <td>
                        ...
                        <tfoot>
                            <tr>
                                ...
                                <th class="bg-grey-1 text-right">{{ moneyFormat(data.total_masuk) }}</th>
                                <th class="bg-grey-1 text-right">{{ moneyFormat(data.total_keluar) }}</th>
                            </tr>
                            <tr>
                                <th class="bg-grey-1" colspan="2"></th>
                                <th class="bg-grey-1">Saldo Kas Akhir</th>
                                <th class="bg-grey-1 text-right">{{ moneyFormat(data.total_masuk - data.total_keluar) }}</th>
                                <th class="bg-grey-1 text-right"></th>

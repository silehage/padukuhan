<script setup>
import { togglePermission } from '@/routes/permissions';
...
const filtered_data = computed(() => {
   ...
   if(filter.value) {
      return props.permissions.filter(el => el.module == filter.value)
   }
               ...
               <table class="table bordered">
                  ...
                  <tbody>
                     <tr v-for="(item, index) in filtered_data" :key="index">
                        ...
                        <td>
                           <div class="nowrap">
                              <span>{{ item.ability }} </span><span v-if="!filter" class="q-ml-sm"> {{ item.module }}</span>
